# Lukas Elsrode - phy_tree.py (08/27/2020)

'''Vizulizes the phylogony of the data used
...
class Trunk:
    def __init__ (self):
        ...
        self.all_fams = [
            ...
            'uraniidae', 'pieridae']
        self.traversal_nodes = [i for i in list(self.graph.nodes) if i not in self.all_fams]
...
class Tree:
    ...
    def complete(self):
        self.fill_tree()
        print('Do you wish you simplify Phylogentic Tree to only non-reducable nodes ? [Y/N]')
        reduce_ = input()
        if reduce_ == 'Y':
            self.simplify_once()
        show_tree(self)
    ...
    def fill_tree(self):
        ...
        # Itterate
        hierarchy = list(self.df.columns[1:5])
        count = 0
        while count < (len(hierarchy) - 1):
            ...
            for p_node in nodes:
                ...
                if p_node != '' and count == 0:
                    ...
                    fams = list(set(self.df.loc[self.df[lvl] == p_node]['family'].values))
                ...
                for child in children:
                    ...
                    if p_node == '' and child != '':
                        grandparent = list(self.df.loc[self.df[sub_lvl] == child][grand_lvl].values)[0]
                        if grandparent != '':
                            self.tree.add_edge(child, grandparent)  ## ++ CHILD <-> GPARENT EDGE
                        else:
                            pass
                    if child == '':
                        dsub_lvl = hierarchy[count + 2]
                        gchildren = list(self.df.loc[self.df[lvl] == p_node][dsub_lvl].values)
                        comp = list(self.df.loc[self.df[sub_lvl] == child][dsub_lvl].values)
                        ...
                        for granchild in gr_children:
                            if granchild != '' and p_node != '':
                                self.tree.add_edge(granchild, p_node) ## ++ G_CHILD <-> PARENT  EDGE
                            else:
                                pass
                        ...
                        if p_node != '':
                            self.tree.add_edge(p_node, child)   ## ++ PARENT <-> CHILD EDGE
                        else:
                            pass
            count += 1
